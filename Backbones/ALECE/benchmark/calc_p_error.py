...
def calc_p_error_my(args, test_wl_type=None, model=None):
    if test_wl_type is None:
        test_wl_type = args.test_wl_type
    if model is None:
        model = args.model
    p_error_dir, _ = "/data1/liuhaoran/ALECE/src/exp/STATS_gpu5_200e_128h/q_error/ins_heavy/"
    fname = 'ALECE_ins_heavy.npy'
    model_errors_path = os.path.join(p_error_dir, fname)
    if os.path.exists(model_errors_path) == False:
        print(f'{model_errors_path} does not exist')
        return None
