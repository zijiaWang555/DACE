...
class queryFeature(object):
    def __init__(self, table_no_map, attr_no_map_list, attr_no_types_list, attr_ranges_list, possible_join_attrs,census=False):
        """
...
def _load_data_from_workload(args, wl_type=None):
    ...
    qF = queryFeature(table_no_map, attr_no_map_list, attr_no_types_list, attr_ranges_list, base_possible_join_attrs,census=False)
...
def load_workload_data_(args):
    ...
    test_sub_features = all_features_2[test_idxes]
    test_sub_cards = all_cards_2[test_idxes]

    valid_idxes = np.where(test_sub_cards >= 0)[0]

    all_train_features, test_sub_features, join_pattern_dim = normalize_data(all_train_features, [test_sub_features],
                                                                             histogram_feature_dim, n_possible_joins)
