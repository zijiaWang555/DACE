...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        #     self.hist_layers=nn.Sequential(
        #         nn.Linear(118,16),
        #         nn.ReLU(),
        #         nn.Linear(16,2)
        #     )
        #     self.hist_add_layers=nn.Sequential(
        #         nn.Linear(44,42)
        #     )
    ...
    def forward(self, x):
        ...
        # if L==2:
        #     ls=open('hist.txt','r')
        #     hist=torch.tensor(list(float(i) for i in ls)).to("cuda")
        #     hist=self.hist_layers(hist)
        #     hist=hist.expand(x.shape[0],hist.shape[0])
        #     x=torch.cat((x,hist),dim=1)
        #     x=self.hist_add_layers(x)
