...
def run_epoch(self,split,
    ...
    losses = []
    tem_data_buffer=queue.Queue()
    if split =='train':
        pass
        # p_weight = p_weight*(math.exp(1.0)/math.exp(epoch_num+1.0))
        # p_weight= p_weight*(1.0/(epoch_num+1.0))
        #p_weight = epoch_num+1.0
        #p_weight=p_weight
    else:
        p_weight=0.0
    if loader is None:
    ...
    model_out_list=torch.empty(1,672)
    model_out_list=model_out_list.to(train_utils.get_device())
    for step, xb in enumerate(loader):
        ...
        for i in range(num_orders_to_forward):
            ...
            model_out = model(xb)
            teacher_1_out=self.teacher_1(xb)
            teacher_2_out=self.teacher_2(xb)
            teacher_3_out=self.teacher_3(xb)
            model_logits.append(model_out)
            teacher_1_logits.append(teacher_1_out)
            teacher_2_logits.append(teacher_2_out)
            teacher_3_logits.append(teacher_3_out)
        ...
        if num_orders_to_forward == 1:
            ...
            if split=='train':
                nll_stduent=loss
                if epoch_num<3:
                    loss = loss + p_weight *F.mse_loss(model_out,teacher_1_out)
                    nll_teacher=self.teacher_1.nll(teacher_1_out, xb, label_smoothing=label_smoothing).mean()
                elif epoch_num<6:
                    loss = loss + p_weight *F.mse_loss(model_out,teacher_2_out)
                    nll_teacher=self.teacher_2.nll(teacher_2_out, xb, label_smoothing=label_smoothing).mean()
                else:
                    loss = loss + p_weight *F.mse_loss(model_out,teacher_3_out)
                    nll_teacher=self.teacher_3.nll(teacher_3_out, xb, label_smoothing=label_smoothing).mean()

                if nll_stduent>nll_teacher:
                    if nll_stduent/nll_teacher > 1.0:
                        data_buffer.put(xb)
                        if data_buffer.qsize() > data_buffer_size:
                            data_buffer.get()
                if epoch_num>8:
                    random_indices = np.random.choice(ot_list.shape[0], size=xbhat.shape[0], replace=False)
                    random_source_xbhat = ot_list[random_indices]
                    loss_ot=self.ot(random_source_xbhat.to(train_utils.get_device()),model_out.to(train_utils.get_device()))
                    if torch.isnan(loss_ot) or torch.isinf(loss_ot):
                        loss_ot=0
                    loss=loss+0.13*loss_ot
        else:
        ...
        #     logps_teacher= []
            ...
            loss = (-logps).mean()
        model_out_list=torch.cat((model_out,model_out_list),dim=0)
    ...
    torch.save(model_out_list,'model_out.pt')
    print("data_buffer_size",data_buffer.qsize())
    while not data_buffer.empty():
        ...
        for i in range(num_orders_to_forward):
            ...
            teacher_2_out=self.teacher_2(xb)
            teacher_3_out=self.teacher_3(xb)
            # print('teacher_1_out:',teacher_1_out)
            model_logits.append(model_out)
            teacher_1_logits.append(teacher_1_out)
            teacher_2_logits.append(teacher_2_out)
            teacher_3_logits.append(teacher_3_out)
        ...
        if num_orders_to_forward == 1:
            ...
            if step<200:
                loss = loss + p_weight *F.mse_loss(model_out,teacher_1_out)
                nll_teacher=model.nll(teacher_1_out, xb, label_smoothing=label_smoothing).mean()
            elif step<400:
                loss = loss + p_weight *F.mse_loss(model_out,teacher_2_out)
                nll_teacher=model.nll(teacher_2_out, xb, label_smoothing=label_smoothing).mean()
            else:
                loss = loss + p_weight *F.mse_loss(model_out,teacher_3_out)
                nll_teacher=model.nll(teacher_3_out, xb, label_smoothing=label_smoothing).mean()

            if nll_stduent>nll_teacher:
                if nll_stduent/nll_teacher > 1.2:
                    tem_data_buffer.put(xb)
                    # print(data_buffer.qsize())
    ...
    if return_losses:
        return losses
    return np.mean(losses),tem_data_buffer
...
class NeuroCard(tune.Trainable):
    ...
    def _setup(self, config):
        ...
        model = self.MakeModel(self.table,
                               self.train_data,
                               table_primary_index=table_primary_index)
        t_hiddens=self.fc_hiddens
        print("self.layers:",self.layers)
        t_layers=self.layers
        t_embed_size=self.embed_size

        self.layers=10
        self.fc_hiddens=128
        self.embed_size=32
        self.teacher_1 = self.MakeModel(self.table,
                               self.train_data,
                               table_primary_index=table_primary_index)
        ...
        self.layers=10
        self.embed_size=32
        self.teacher_2 = self.MakeModel(self.table,
                               self.train_data,
                               table_primary_index=table_primary_index)
        ...
        self.layers=10
        self.emebd_size=32
        self.teacher_3 = self.MakeModel(self.table,
                               self.train_data,
                               table_primary_index=table_primary_index)
        ...
        self.ot=OT('cpu')
        self.ot_list=torch.load('model_out.pt')
        self.ot_list.to(train_utils.get_device())
    ...
    def LoadCheckpoint_n(self,model,path):
    ...
    def _simple_save(self):
        ...
        path='/data1/jxlei/NeuroCard/neurocard/neurocard/models'
    ...
    def _train(self):
        ...
        for _ in range(min(self.epochs - self.epoch,
            ...
            mean_epoch_train_loss,self.data_buffer = run_epoch(self,
                'train',
                self.model,
                self.opt,
        ...
        if self.compute_test_loss:
            returns['test_bits'],self.data_buffer = run_epoch(self,
                    'test',
                    self.model,
                    opt=None,
                    train_data=self.train_data,
            ...
            returns['test_bits']=returns['test_bits']/np.log(2)
    ...
    def _save(self, tmp_checkpoint_dir):
        ...
        # PATH='/data1/jxlei/NeuroCard/neurocard/neurocard/models/teacher.pt'
        print('model_type:',type(self.model))
