            ...
            for o, v in zip(op, val):
                if col.factor_id is None:
                    ...
                    #   everything.
                        ...
                        if o in ['<=', '<', '!='] and np.any(pd.isnull(columns[j].all_distinct_values)):
                            fact_ops[i].append(col.ProjectOperator('>'))
                            fact_vals[i].append(col.ProjectValue(0))
                            fact_dominant_ops[i].append(col.ProjectOperatorDominant('>'))
                        if v not in columns[j].all_distinct_values:
                            ...
                            if o == '=':
                                ...
                                if o in ['<', '<=']:
                                    if v < columns[j].all_distinct_values[1]:
                                        fact_ops[i].append('ALL_FALSE')
                                        fact_vals[i].append(None)
                                        fact_dominant_ops[i].append(None)
                                    elif v > columns[j].all_distinct_values[-1]:
                                        fact_ops[i].append('ALL_TRUE')
                                        fact_vals[i].append(None)
                                        fact_dominant_ops[i].append(None)
                                    else:
                                        idx = np.searchsorted(columns[j].all_distinct_values, v, side='right')
                                        new_v = columns[j].all_distinct_values[idx]
                                        new_op = '<='
                                        value = np.nonzero(
                                            columns[j].all_distinct_values == new_v)[0][0]
                                        p_v = col.ProjectValue(value)
                                        p_op = col.ProjectOperator(new_op)
                                        p_dom_op = col.ProjectOperatorDominant(new_op)
                                    ...
                                    if v > columns[j].all_distinct_values[-1]:
                                        ...
                                        idx = np.searchsorted(columns[j].all_distinct_values, v, side='left')
                                        new_v = columns[j].all_distinct_values[idx]
                                        new_op = '>='
                                        value = np.nonzero(
                                            columns[j].all_distinct_values == new_v)[0][0]
...
class BatchDifferentiableProgressiveSampling(CardEst):
    ...
    def _sample_n(self,
        ...
        for i in range(ncols):
            ...
            # shape (batch_size * num_samples, num_classes)
            # print("logits_i.shape:",logits_i.shape)
            samples_i_batch = torch.nn.functional.gumbel_softmax(logits_i_n, tau=self.tau, hard=True)
