...
def run_epoch(self,
    ...
    if split =='train': 
        kd_weight = kd_weight*(math.exp(1.0)/math.exp(epoch_num+1.0))
    else:
        kd_weight=0.0
    ...
    for step, xb in enumerate(loader):
        ...
        if num_orders_to_forward == 1:
            ...
            nll_teacher=0
            if step<200:
                loss = loss + kd_weight *F.mse_loss(model_out,teacher_1_out)
                nll_teacher=model.nll(teacher_1_out, xb, label_smoothing=label_smoothing).mean()
            elif step<400:
                loss = loss + kd_weight *F.mse_loss(model_out,teacher_2_out)
                nll_teacher=model.nll(teacher_2_out, xb, label_smoothing=label_smoothing).mean()
            else:
                loss = loss + kd_weight *F.mse_loss(model_out,teacher_3_out)
...
class NeuroCard(tune.Trainable):
    ...
    def _setup(self, config):
        ...
        # print("teacher_2 paramaters",sum(p.numel() for p in self.teacher_2.parameters()))
        # print("teacher_3 paramaters",sum(p.numel() for p in self.teacher_3.parameters()))
        ...
        if self.dataset == 'imdb' and len(self.join_tables) > 1:
            ...
            # print('queries_job_format:',queries_job_format)
        ...
        print("loaded_queries",type(self.loaded_queries))
        if config['__gpu'] == 0:
