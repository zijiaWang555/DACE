...
def run_epoch(split,
    ...
    for step, xb in enumerate(loader):
        ...
        if num_orders_to_forward == 1:
            ...
            # if transfer and epoch_num > pretrain_epoch:
                # loss += 0.07 * ot(random_source_xbhat, model_out)
    ...
    def _setup(self, config):
        ...
        if self.dataset == 'imdb':
            ...
            for t in self.join_tables:
                print('Loading', t)
                table = datasets.LoadImdb(t, data_dir=self.target_dataset_dir,use_cols=self.use_cols)
                table.data.info()
        ...
        if self.transfer:
            # filename=os.path.join('/data1/jxlei/UAE/UAE_joins/source_interv','source_interv_{}.pkl'.format(1))
            # source_interv=np.load(filename,allow_pickle=True)
            # for i in range(8):
            #     filename=os.path.join('/data1/jxlei/UAE/UAE_joins/source_interv','source_interv_{}.pkl'.format(i+2))
            #     source_interv=np.concatenate((source_interv,np.load(filename,allow_pickle=True)),axis=0)
            # self.source_interv=source_interv
            # self.ot = OT(train_utils.get_device())
            self.ot=None
            self.source_interv=None
        ...
        for _ in range(min(self.epochs - self.epoch,
            ...
            if self.run_uaeq:
                ...
                mean_epoch_train_loss = run_epoch(
                    ...
                    source_interv = self.source_interv,
                    ot=self.ot                    
                )
            self.epoch += 1
